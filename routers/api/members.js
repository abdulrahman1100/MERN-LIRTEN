  ...
  if (project) {
    ...
    if (member) {
      if (member.experienceLevel >= project.experienceLevel) {
        ...
        if (project.appliedMembers.filter(e => e.id == memberId).length > 0) return res.status(400).send({ error:'Applied Before'})
  ...
  if (member) {
    if (course) {
      if (course.availablePlaces > 0 & course.isAvailable) {
        ...
        if (course.listOfApplied.filter(e => e.id == memberId).length > 0) return res.status(400).send({ error:'Applied Before'})
      ...
      } else return response.status(404).send({ error:'this course is not availabe right now its Full'});     
    } else return response.status(404).send({ error:'there is not such course in the site'}); 
  } else return response.status(404).send({ error: 'there is not such member in the site'});
  ...
  if (member) {
    if (masterClass) {
      if (masterClass.availablePlaces > 0 & masterClass.isAvailable) {
        ...
        if (masterClass.listOfApplied.filter(e => e.id == memberId).length > 0) return res.status(400).send({ error:'Applied Before'})
        ...
        res.sendStatus(200);
      } else return res.status(404).send({ error:'this masterClass is not availabe right now its Full'});
    } else return res.status(404).send({ error:'there is not such masterClass in the site'});  
  } else return res.status(404).send({ error:'Your are not member in the site'});
