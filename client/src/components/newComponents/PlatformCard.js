...
class PlatformCard extends Component {
  ...
  rate0 (){
    ...
    axios
        .put(`https://hakunamatatasite.herokuapp.com/api/courseRequests/raterecomendation/${this.props.objectId}`,data)
        .then( res=>{
          let arr=[]
          arr.push(0)
  ...
  rate3(){
    ...
    axios
      ...
      react0(){
        ...
        axios
            .put(`https://hakunamatatasite.herokuapp.com/api/posts/ratePost/${this.props.objectId}`,data)
            .then( res=>{
              let arr=[]
              arr.push(0)
              this.setState({temp:arr})
  ...
  componentDidMount() {
    ...
    if (this.props.tag === 'CourseRequest'){
      axios
        .get(`https://hakunamatatasite.herokuapp.com/api/courseRequests/${this.props.objectId}`)
        .then(res => {
      ...
      if (this.props.tag === 'Post'){
        // console.log(this.props.objectId)
        axios
          .get(`https://hakunamatatasite.herokuapp.com/api/posts/${this.props.objectId}`)
          .then(res => {
            if (res.data.data != null) {
  ...
  onClickView() {
    if (this.props.type != undefined & this.props.objectId != undefined) window.location.href = `https://hakunamatatasite.herokuapp.com/${this.props.type}/${this.props.objectId}`
  }
  handleSubmit(){
    let data=null
    const id=this.props.auth.user._id
    ...
    console.log(this.props.objectId)
    axios
        .put(`https://hakunamatatasite.herokuapp.com/api/courseRequests/giveRecomendation/${this.props.objectId}`,data)
        .then( res=>{
  ...
  handleSubmitComment(){
    ...
    axios
        .put(`https://hakunamatatasite.herokuapp.com/api/posts/giveComment/${this.props.objectId}`,data)
        .then( res=>{
          const temp=this.state.comments.unshift(<h6>test</h6>)
          this.setState({comments:temp})
        }).catch(error => {
